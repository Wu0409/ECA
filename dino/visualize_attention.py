...
import sys
sys.path.append('/media/store/wyc/exps/T_Wseg')
...
if __name__ == '__main__':
    ...
    parser.add_argument("--image_path", default='../attn_vis/img2.jpg', type=str, help="Path of the image to load.")
    ...
    attentions = attentions[0, :, 1780, 1:].reshape(nh, -1)
    ...
    for j in range(nh):
        plt.imshow(attentions[j], interpolation='nearest')
        plt.title(f"Attention Head {j}")
        plt.axis('off')
        plt.show()  # 不保存，直接显示输出
