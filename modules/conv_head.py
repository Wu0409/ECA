...
class LargeFOV(nn.Module):
    def __init__(self, in_planes, out_planes, dilation=5):
        ...
        self.conv6 = conv3x3(in_planes=in_planes, out_planes=self.embed_dim, padding=self.dilation,
                             dilation=self.dilation)
        self.relu6 = nn.ReLU(inplace=True)

        self.conv7 = conv3x3(in_planes=self.embed_dim, out_planes=self.embed_dim, padding=self.dilation,
                             dilation=self.dilation)
