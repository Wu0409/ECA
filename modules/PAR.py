...
class PAR(nn.Module):
    ...
    def forward(self, imgs, masks):
        ...
        b, c, h, w = imgs.shape
        _imgs = self.get_dilated_neighbors(imgs)  # torch.Size([1, 3, 160, 160])
...
class PAR_rv(nn.Module):
    ...
    def forward(self, imgs, masks, atts):
        masks = F.interpolate(masks, size=imgs.size()[-2:], mode="bilinear", align_corners=True)
        ...
        # My exp --------------------------------
        _atts = self.get_dilated_neighbors(atts)

        _atts_rep = atts.unsqueeze(self.dim).repeat(1, 1, _atts.shape[self.dim], 1, 1)
        _atts_abs = torch.abs(_atts - _atts_rep)
        _atts_std = torch.std(_atts, dim=self.dim, keepdim=True)
        att_aff = -(_atts_abs / (_atts_std + 1e-8) / self.w1) ** 2
        ...
        aff = F.softmax(aff, dim=2) + 0.4 * F.softmax(att_aff, dim=2)
