...
def train(args):
    ...
    val_dataset = voc.VOC12SegDataset(
        root_dir=args.root_dir, name_list_dir=args.name_list_dir, split='val',
        stage='val', aug=False,
    )
    ...
    # SegFormer
    model = WeTr(
        backbone='mit_b1', stride=[4, 2, 2, 1], num_classes=21,
        pretrained_dir='../pretrained/mit_b1.pth',  # put the backbone weight here
    ...
    state_dict = torch.load('../pretrained/dino_deitsmall{}_pretrain.pth'.format(args.patch_size), map_location="cpu")
    ...
    memobank = torch.randn(20, 512).to(device)
    ...
    for n_iter in range(args.max_iter):
        ...
        if n_iter <= args.cam_iters:
            ...
            cls_labels_cam = train_helper.convert_seg2image_cls(cam_pseudo_label, args)

            dino_pseudo_label = get_pseudo_labels_rv_cam_10_4(
                model_dino, inputs, cls_labels_cam, cams, reps, par, [1.0, 0.5], img_box, memobank, args
            )
            ...
            seg_loss_d = train_helper.get_seg_loss(segs, dino_pseudo_label.type(torch.long), ignore_index=255)

            # Segmentation Loss (CAM)
            # ------------------------------------------------------------------------------------------------
            segs = F.interpolate(segs, size=cam_pseudo_label.shape[1:], mode='bilinear', align_corners=False)
            if n_iter <= 6000:
                seg_loss_c = train_helper.get_seg_loss(segs, cam_pseudo_label.type(torch.long), ignore_index=255)
            else:
                ...
                seg_loss_c = train_helper.get_seg_loss(segs, cam_pseudo_label_aff.type(torch.long), ignore_index=255)
            ...
            alignment_loss = train_helper.AlignmentLoss(args)
            ...
            # ------------------------------------------------------------------------------------------------
            if n_iter <= 4000:
                loss = 1.0 * cls_loss + 0.1 * seg_loss_d + 0.0 * seg_loss_c + (args.w_aff * aff_loss) + (0.0 * ali_loss)
            else:
                loss = 1.0 * cls_loss + 0.1 * seg_loss_d + 0.1 * seg_loss_c + (args.w_aff * aff_loss) + (args.w_ali * ali_loss)
        ...
        if local_rank == 0:
            ...
            if (n_iter + 1) % 2000 == 0:
                ...
                cls_score, seg_score, cam_score, aff_score = train_helper.validate_v811(
    ...
    if local_rank == 0:
        ...
        tblogger.add_text('\nfinal_seg_score', str(seg_score))
        tblogger.add_text('\nfinal_msc_seg_score', str(msc_seg_score))

        # NOTE: the final results is without post-processing
...
def get_config():
    ...
    # Setup
    parser.add_argument("--work_dir", default='./work_dir_voc/', type=str, help="work_dir")
    ...
    parser.add_argument("--comment", default='voc_final', type=str, help="comment")
    ...
    # Dataset
    parser.add_argument("--root_dir", default="/media/store/wyc/Dataset/VOC2012/images", type=str)
    parser.add_argument("--name_list_dir", default="datasets/voc", type=str)
    ...
    # Train
    parser.add_argument("--split", default="train_aug", type=str)
    ...
    parser.add_argument("--max_iter", default=18000, type=int)
    parser.add_argument("--cam_iters", default=2000, type=int)
