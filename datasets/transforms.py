...
class GaussianBlur(object):
    ...
    def __init__(self, p=0.5):
        self.prob = p
        self.blur_operation = transforms.GaussianBlur(kernel_size=5)
    ...
    def __call__(self, img):
        ...
        return self.blur_operation(img)


def random_crop_3(image, label=None, sals=None, crop_size=None, mean_rgb=[0, 0, 0], ignore_index=255):
    h, w, _ = image.shape
    ...
    pad_sal = np.ones((H, W), dtype=np.float32) * ignore_index
    pad_sal[H_pad:(H_pad + h), W_pad:(W_pad + w)] = sals
    sal = pad_sal[H_start:H_end, W_start:W_end]

    return image, label, sal, img_box


def random_scaling_3(image, label=None, sal=None, scale_range=None):
    min_ratio, max_ratio = scale_range
    assert min_ratio <= max_ratio

    ratio = random.uniform(min_ratio, max_ratio)

    return _img_rescaling_3(image, label, sal, scale=ratio)


def _img_rescaling_3(image, label=None, sal=None, scale=None):
    # scale = random.uniform(scales)
    h, w, _ = image.shape
    ...
    new_sals = Image.fromarray(sal).resize(new_scale, resample=Image.NEAREST)
    new_sals = np.asarray(new_sals)

    return new_image, new_label, new_sals
