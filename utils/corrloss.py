...
class ContrastiveCorrelationLoss_v2(nn.Module):
    def __init__(self, sample_num_local=25, sample_num_global=25):
        super(ContrastiveCorrelationLoss_v2, self).__init__()
        self.inter_cal = None
        self.intra_cal = None
        self.neg_cal = None

        self.feature_samples_local = sample_num_local
        self.feature_samples_global = sample_num_global
    ...
    def forward(self,
        ...
        coord_shape_local = [orig_feats.shape[0], self.feature_samples_local, self.feature_samples_local, 2]
        coord_shape_global = [orig_feats.shape[0], self.feature_samples_global, self.feature_samples_global, 2]

        coords1 = torch.rand(coord_shape_local, device=orig_feats.device) * 2 - 1
        coords2 = torch.rand(coord_shape_global, device=orig_feats.device) * 2 - 1
